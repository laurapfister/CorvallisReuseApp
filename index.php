<?php
	...
	$app->delete('/repair/:name', function($name) use ($app){
		$mysqli = new mysqli("mysql.eecs.oregonstate.edu", "cs419-g4", "RNjFRsBYJK5DVF8d", "cs419-g4");
		$stmt = $mysqli->prepare("DELETE FROM repair_businesses WHERE ?");
		$stmt->bind_param("s", $name);
		$stmt->execute();
		$mysqli->close();
	});
	...
	$app->delete('/repairItem/:item', function($item) use($app){
		$mysqli = new mysqli("mysql.eecs.oregonstate.edu", "cs419-g4", "RNjFRsBYJK5DVF8d", "cs419-g4");
		$stmt = $mysqli->prepare("DELETE FROM repair_items WHERE ?");
		$stmt->bind_param("s", $item);
		$stmt->execute();
		$mysqli->close();
	});
	...
	$app->put('/repair/:name/:item', function($name, $item){
		$mysqli = new mysqli("mysql.eecs.oregonstate.edu", "cs419-g4", "RNjFRsBYJK5DVF8d", "cs419-g4");
		$stmt = $mysqli->prepare("INSERT INTO rep_bus_items(bId, iId) VALUES((SELECT repairId WHERE repairName = ?), (SELECT itemId FROM repair_items WHERE ItemName = ?))");
		$stmt->bind_param("ss", $name, $item);
		$mysqli->execute();
		$mysqli->close();
	});
